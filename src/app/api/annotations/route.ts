...
export async function GET(request: NextRequest) {
  try {
    ...
    const annotations = await query(sql, params);

    // Transform annotations to match frontend format
    const transformed = annotations.map((ann: any) => ({
      id: ann.id,
      documentId: ann.document_id,
      type: ann.type,
      page: ann.page,
      position: {
        x: parseFloat(ann.position_x),
        y: parseFloat(ann.position_y),
        ...(ann.position_width && { width: parseFloat(ann.position_width) }),
        ...(ann.position_height && { height: parseFloat(ann.position_height) }),
        ...(ann.position_radius && { radius: parseFloat(ann.position_radius) }),
        ...(() => {
        ...
        ...(ann.position_start_angle && { startAngle: parseFloat(ann.position_start_angle) }),
        ...(ann.position_end_angle && { endAngle: parseFloat(ann.position_end_angle) }),
      ...
      style: {
        ...
        opacity: parseFloat(ann.style_opacity),
        ...(ann.style_stroke_width && { strokeWidth: parseFloat(ann.style_stroke_width) }),
      ...
      metrics: {
        ...(ann.metrics_area && { area: parseFloat(ann.metrics_area) }),
        ...(ann.metrics_perimeter && { perimeter: parseFloat(ann.metrics_perimeter) }),
        ...(ann.metrics_length && { length: parseFloat(ann.metrics_length) }),
        ...(ann.metrics_radius && { radius: parseFloat(ann.metrics_radius) }),
        ...(ann.metrics_rx && { rx: parseFloat(ann.metrics_rx) }),
        ...(ann.metrics_ry && { ry: parseFloat(ann.metrics_ry) }),
        ...(ann.metrics_area_px && { area_px: parseFloat(ann.metrics_area_px) }),
        ...(ann.metrics_length_px && { length_px: parseFloat(ann.metrics_length_px) }),
