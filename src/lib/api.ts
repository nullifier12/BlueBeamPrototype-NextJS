...
export async function login(username: string, password: string, projectId?: string) {
  return apiRequest<{ success: boolean; user: User; projectId?: string }>('/auth/login', {
    method: 'POST',
...
export async function getDocuments(projectId: string) {
  return apiRequest<{ documents: PDFDocument[] }>(`/documents?projectId=${projectId}`);
}

export async function createDocument(data: Partial<PDFDocument> & { 
  projectId: string; 
  name: string; 
  fileSize: number;
  filePath?: string;
...
export async function createAnnotation(data: Partial<Annotation> & { documentId: string; projectId: string; type: string; page: number; position: { x: number; y: number }; style: { color: string; opacity: number } }) {
...
export async function getPunchItems(projectId: string) {
  return apiRequest<{ punchItems: PunchListItem[] }>(`/punch-items?projectId=${projectId}`);
}

export async function createPunchItem(data: Partial<PunchListItem> & { projectId: string; description: string }) {
...
export async function updatePunchItem(id: string, data: Partial<PunchListItem>) {
